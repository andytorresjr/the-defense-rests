...
const CONTESTED = ['identity', 'timeline', 'intent', 'weapon', 'altSuspect'];

export function updateFocus(state) {
  if (!state.jury.length) return null;
  const means = meanBeliefs(state);
  // Lowest mean = the issue most eroded by the defense.
  const focus = CONTESTED.slice().sort((a, b) => (means[a] ?? 0.5) - (means[b] ?? 0.5))[0];
...
export const FOCUS_LABEL = {
  identity: 'who was in that lot',
  timeline: 'when the defendant was there',
  intent: 'why Marcus Webb died',
  weapon: 'the knife',
  altSuspect: 'this phantom alternative suspect',
};
